#################################
####### Safelevel - SMOTE #######
## data_origin - training dataset (with SMOTE already applied)
## target - target variable
## n - splits in each iteration
## k - stopping_rounds where rejected obs<=p
## p - percentage from data_origin to use as stopping_metric
...
smote_ipf <- function(data_origin, target, n=9, k=3, p=0.01, silent=TRUE) {
  ...
  if(missing(data_origin) | missing(target)) {
    stop("missing data_origin or target")
  }
  
  colclasses <- sapply(data_origin, class)
  ...
  for (conv in char2fac) {
    if (class(data_origin[,conv])=="character") data_origin[,conv] <- as.factor(data_origin[,conv])
  }
  
  # initialise
  E <- data_origin
  E$row_id <- 1:nrow(E)
  colnames(E)[colnames(E)==target] <- "smote_ipf_target"
  k_i <- 1
  eval_k_i <- TRUE
  
  # create model formula
  features <- colnames(data_origin)[colnames(data_origin)!=target]
  full_formula <- as.formula(paste0("smote_ipf_target", " ~ ", paste(features, collapse = " + ")))
  
  while(eval_k_i) {
    # first rearrange data_origin via sample
    # then rep_len(c(1:n), length.out = n())
    set.seed(k_i)
    E <- E[sample(x = 1:nrow(E), replace = FALSE), ]
    E$split_k_i <- rep_len(c(1:n), length.out = nrow(E))
    
    for (n_i in 1:n) {
      E_n_i <- E[E$split_k_i==n_i,]
      ...
      predict_n_i <- E %>%
        ...
        mutate(label=predict(c45_n_i, newdata=E, type="class")) 
      
      if(n_i==1) {
        predict_k_i <- predict_n_i
      } else {
        predict_k_i <- predict_k_i %>% bind_rows(predict_n_i)
      }
    ...
    p_k_i <- nrow(cast_vote_k_i)/nrow(data_origin)
    E <- E %>% 
  ...
  colnames(E)[colnames(E)=="smote_ipf_target"] <- target
  return(E)
}
